...
use pi_ai::Message;
use pi_coding_agent::session::{SessionEntry, SessionManager};
use std::fs;
...
async fn branch_merge_export_workflow() {
    ...
    let m1 = manager
        .append_message(AgentMessage::from_llm(Message::user("Initial setup")))
        .await
        .expect("m1");
    ...
    manager.branch(&m1);
    let m2 = manager
        .append_message(AgentMessage::from_llm(Message::user("Main branch work")))
        .await
        .expect("m2");
    ...
    manager.branch(&m1);
    let m3 = manager
        .append_message(AgentMessage::from_llm(Message::user("Side branch feature")))
        .await
        .expect("m3");
    ...
    assert_eq!(
        m1_node.children.len(),
        2,
        "m1 should have 2 children (branches)"
    );

    // Step 4: Navigate to main branch and continue
    manager.branch(&m2);
    let _m5 = manager
        .append_message(AgentMessage::from_llm(Message::user("Back on main")))
        .await
        .expect("m5");
    ...
    let tree = manager
        .get_tree()
        .await
        .expect("get tree after more branching");
...
async fn session_file_merge_workflow() {
    ...
    assert_eq!(
        tree.len(),
        4,
        "Should have 4 entries total (2 target + 2 source)"
    );
    ...
    let messages = merge_manager
        .navigate_to(&last_entry)
        .await
        .expect("navigate to last entry");
    assert!(
        !messages.is_empty(),
        "Should have messages in navigation path"
    );
...
async fn large_session_stress_test() {
    ...
    for i in 0..entry_count {
        ...
        manager
            .append_message(AgentMessage::from_llm(Message::user(&msg)))
            .await
    ...
    let messages = manager
        .navigate_to(&last_id)
        .await
        .expect("navigate to last");
    let nav_duration = nav_start.elapsed();

    assert_eq!(
        messages.len(),
        entry_count,
        "Should have all messages in navigation"
    );
    ...
    let _first_messages = manager
        .navigate_to(&first_id)
        .await
        .expect("navigate to first");
