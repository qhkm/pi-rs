...
async fn main() -> Result<()> {
    ...
    match args.command {
        Some(PodCommand::Setup { name, ssh, mount, vllm }) => {
        ...
        Some(PodCommand::Start { model, name, memory, context, gpus }) => {
...
async fn cmd_list() -> Result<()> {
    ...
    println!("{:<15} {:<25} {:<10} {:<20}", "NAME", "SSH", "GPUS", "RUNNING MODELS");
    println!("{}", "-".repeat(80));
...
async fn cmd_active(name: String) -> Result<()> {
    ...
    if !config.pods.contains_key(&name) {
        anyhow::bail!("Pod '{}' not found. Run 'pi-pods list' to see available pods.", name);
    }
...
async fn cmd_start(
    ...
    let mut config = Config::load()?;
    let pod_name = config.active_pod()
        .map(|(n, _)| n.to_string())
    ...
    let (pod_ssh, pod_vllm_version) = {
        let pod = config.pods.get(&pod_name)
    ...
    let pid = vllm.start_model(&model, port, &gpu_ids, memory.as_deref(), context.as_deref()).await?;
...
async fn cmd_stop(name: String) -> Result<()> {
    ...
    let pod_name = if name.is_empty() {
        config.active_pod()
            .map(|(n, _)| n.to_string())
            .ok_or_else(|| anyhow::anyhow!("No active pod. Use 'pi-pods stop <name>' to specify."))?
    } else {
    ...
    let pod = config.pods.get_mut(&pod_name)
    ...
    let running = pod.models.get(&model_name)
        .ok_or_else(|| anyhow::anyhow!("Model '{}' is not running on pod '{}'", model_name, pod_name))?;
...
async fn cmd_logs(name: &str) -> Result<()> {
    let config = Config::load()?;
    let (pod_name, pod) = config.active_pod()
        .ok_or_else(|| anyhow::anyhow!("No active pod"))?;
    
    let running = pod.models.get(name)
...
async fn cmd_shell(name: &str) -> Result<()> {
    ...
    let ssh = if name.is_empty() {
        config.active_pod()
            .map(|(_, p)| p.ssh.clone())
            .ok_or_else(|| anyhow::anyhow!("No active pod. Use 'pi-pods shell <pod-name>' to specify."))?
    } else {
        config.pods.get(name)
            .map(|p| p.ssh.clone())
